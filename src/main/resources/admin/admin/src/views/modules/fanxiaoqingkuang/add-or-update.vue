<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="返校状态" prop="fanxiaozhuangtai">
          <el-select v-model="ruleForm.fanxiaozhuangtai" placeholder="请选择返校状态">
            <el-option
                v-for="(item,index) in fanxiaozhuangtaiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="返校状态" prop="fanxiaozhuangtai">
	      <el-input v-model="ruleForm.fanxiaozhuangtai"
                placeholder="返校状态" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='fanxiaozhuangtai'){
            this.ruleForm.fanxiaozhuangtai = obj[o];
	    this.ro.fanxiaozhuangtai = true;
            continue;
          }
          if(o=='beizhu'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `fanxiaoqingkuang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.fanxiaoqingkuangCrossAddOrUpdateFlag = false;
                  this.parent.search();
